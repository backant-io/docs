...
Everything to do with memory, dream cycles, and policy lives on your machine.
...
| `~/.claude/kairos/.session/` | Cycle-scoped scratch state. |
| `~/.claude/kairos/.state/` | Daemon state (PID, current cycle, etc.). |
...
1. **Claude API calls** — required for reasoning. Without these, no cycles run.
