---
title: "Memory"
description: "Inspect and manage the local memory store. Recall is automatic during cycles — these commands are for introspection."
---
...
The daemon recalls automatically during cycles. These commands are for introspection, debugging, and one-off cleanups.
...
Each hit shows a score (combining lexical, semantic, and lifecycle signals), tier, type, ID, and content preview. The agent uses the same recall surface during cycles via MCP.
...
| Tier | Purpose | Decay |
|---|---|---|
| **STM** (short-term) | In-session observations, retries, anomalies. Recent ground truth. | Fast. Entries that aren't reinforced collapse below the archive threshold within a small number of cycles. |
